...
func run() error {
	...
	err := filepath.WalkDir(".", func(path string, entry fs.DirEntry, err error) error {
		...
		if entry.IsDir() {
			if name == "." || name == "openapi" || name == "migration" || strings.HasPrefix(name, "migration/") || name == "toolkit" || strings.HasPrefix(name, "toolkit/") || name == "internal" || name == "internal/validationrules" || name == "internal/swaggerui" || name == "internal/swaggerui/assets" {
				return nil
			}
